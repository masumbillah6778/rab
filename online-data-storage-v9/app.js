...
function updateAutocompleteSuggestions() {
  const fields = [
    ...
    ["cardNumber", "cardNumberSuggestions"],
    ...
    ["lastWithdrawnRation", "lastWithdrawnRationSuggestions"],
...
function render() {
  ...
  $("recordsBody").innerHTML = list.map((r,i)=>`
    <tr>
      ...
      <td>${r.personalNumber||""}</td><td>${r.rabId||""}</td><td>${r.rationNumber||""}</td><td>${r.cardNumber||""}</td>
      <td>${r.rank||""}</td><td>${r.name||""}</td><td>${r.workplace||""}</td>
      <td>${r.transferPlace||""}</td><td>${r.date||""}</td><td>${r.lastWithdrawnRation||""}</td><td>${formatDateTime(r.createdAt)}</td><td>${r.memorandumNumber||""}</td>
...
$("cancelBtn").onclick = () => { resetRecordForm(); hide("formCard"); };

$("recordForm").onsubmit = async e => {
  e.preventDefault();
  const numeric = ["personalNumber", "rabId", "rationNumber", "cardNumber", "mobileNumber"];
  for (const id of numeric) {
    const v = $(id).value.trim();
    if (!/^\d{4,12}$/.test(v)) return msg("formMessage", `${id} ৪ থেকে ১২ সংখ্যার মধ্যে লিখুন।`);
  }
  const data = {
    ownerUid: currentUser.uid, personalNumber:$("personalNumber").value.trim(), rabId:$("rabId").value.trim(),
    rationNumber:$("rationNumber").value.trim(), cardNumber:$("cardNumber").value.trim(), rank:$("rank").value, name:$("name").value,
    workplace:$("workplace").value, transferPlace:$("transferPlace").value, date:$("date").value,
    lastWithdrawnRation:$("lastWithdrawnRation").value,
    memorandumNumber:$("memorandumNumber").value, mobileNumber:$("mobileNumber").value.trim()
  };
  ...
  hide("formCard");
  await loadRecords();
  msg("formMessage","সংরক্ষিত হয়েছে।",true);
};
...
function openProfileModal(r) {
  ...
  const fields = [
    ...
    ["কার্ড নং", r.cardNumber],
    ["পদবী", r.rank],
    ["নাম", r.name],
    ["কর্মস্থল", r.workplace],
    ["বদলীস্থল", r.transferPlace],
    ["তারিখ", r.date],
    ["সর্বশেষ উত্তোলিত রেশন", r.lastWithdrawnRation],
...
function resetRecordForm() {
  ...
  $("recordId").value = "";
  editingId = null;
  $("formTitle").textContent = "নতুন ডাটা যোগ করুন";
}
window.editRecord = id => {
  const r=records.find(x=>x.id===id);
  if (!r) return;
  editingId=id;
  for(const k of ["personalNumber","rabId","rationNumber","cardNumber","rank","name","workplace","transferPlace","date","lastWithdrawnRation","memorandumNumber","mobileNumber"]) $(k).value=r[k]||"";
  $("formTitle").textContent="ডাটা সম্পাদনা";
  show("formCard");
};
...
$("csvExportBtn").onclick=()=>{const keys=['personalNumber','rabId','rationNumber','cardNumber','rank','name','workplace','transferPlace','date','lastWithdrawnRation','memorandumNumber','mobileNumber','createdAt'];downloadFile('ordms-data.csv',keys.join(',')+'\n'+records.map(r=>keys.map(k=>'"'+String(k==='createdAt'?formatDateTime(r[k]):(r[k]||'')).replaceAll('"','""')+'"').join(',')).join('\n'),'text/csv')};
...
pageButtons.forEach(b=>b.onclick=()=>{ document.querySelectorAll('#appSection>section, #appSection>.dashboard, #appSection>.toolbar').forEach(x=>x.classList.add('hidden')); const p=b.dataset.page; setActivePage(p); if(p==='backup') show('backupPage'); else if(p==='form') { resetRecordForm(); show('formCard'); } else if(p==='settings') show('settingsPage'); else if(p==='records'){ show('recordsToolbar'); document.querySelector('.table-card').classList.remove('hidden'); } else { document.querySelector('.dashboard').classList.remove('hidden'); } });
