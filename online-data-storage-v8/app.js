...
$("recordForm").onsubmit = async e => {
  e.preventDefault();
  const numeric = ["personalNumber", "rabId", "rationNumber", "mobileNumber"];
  ...
  const data = {
    ...
    rationNumber:$("rationNumber").value.trim(), rank:$("rank").value, name:$("name").value,
...
window.editRecord = id => {
  ...
  if (!r) return;
  editingId=id;
  for(const k of ["personalNumber","rabId","rationNumber","rank","name","workplace","transferPlace","date","lastWithdrawnRation","memorandumNumber","mobileNumber"]) $(k).value=r[k]||"";
...
$("csvExportBtn").onclick=()=>{const keys=['personalNumber','rabId','rationNumber','rank','name','workplace','transferPlace','date','lastWithdrawnRation','memorandumNumber','mobileNumber','createdAt'];downloadFile('ordms-data.csv',keys.join(',')+'\n'+records.map(r=>keys.map(k=>'"'+String(k==='createdAt'?formatDateTime(r[k]):(r[k]||'')).replaceAll('"','""')+'"').join(',')).join('\n'),'text/csv')};
