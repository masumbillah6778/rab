...
function speakBangla(text) {
  ...
  try {
    ...
    const banglaVoice = voices.find(v => /^bn(-|_)/i.test(v.lang));
    if (banglaVoice) utterance.voice = banglaVoice;
    window.speechSynthesis.speak(utterance);
  } catch (e) { console.warn("Bangla audio unavailable", e); }
}
...
async function unlock() {
  try {
    ...
    if (!saved) {
      ...
      speakBangla("আপনাকে স্বাগতম।");
    } else if (saved !== password) {
      msg("protectorMessage", "ভুল পাসওয়ার্ড।");
      speakBangla("দুঃখিত, আপনার পাসওয়ার্ড ভুল হয়েছে।");
      return;
    } else {
      speakBangla("আপনাকে স্বাগতম।");
    }
...
function clearEntryForm() {
  ...
  if (form) form.reset();
  editingId = null;
  $("formTitle").textContent = "নতুন তথ্য এন্ট্রি";
  msg("formMessage", "");
}
$("cancelBtn").onclick = () => { clearEntryForm(); hide("formCard"); };
...
pageButtons.forEach(b=>b.onclick=()=>{ document.querySelectorAll('#appSection>section, #appSection>.dashboard, #appSection>.toolbar').forEach(x=>x.classList.add('hidden')); const p=b.dataset.page; setActivePage(p); if(p==='backup') show('backupPage'); else if(p==='form'){ clearEntryForm(); show('formCard'); } else if(p==='settings') show('settingsPage'); else if(p==='records'){ show('recordsToolbar'); document.querySelector('.table-card').classList.remove('hidden'); } else { document.querySelector('.dashboard').classList.remove('hidden'); } });
